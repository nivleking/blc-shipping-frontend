...
const WeeklyPerformancePanel = ({ roomId, totalRounds = 1 }) => {
    ...
    <div className="space-y-4">
      ...
      {selectedUserId ? (
        <div className="bg-white rounded-lg shadow p-4">
          <WeeklyPerformance userId={selectedUserId} isAdminView={true} totalRounds={totalRounds + 1} currentRound={totalRounds + 1} showFinancialModal={false} />
        </div>
      ) : (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
          <p className="text-gray-500">Select a participant to view weekly performance data</p>
        </div>
      )}
    </div>
  );
